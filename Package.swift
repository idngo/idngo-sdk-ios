...
let package = Package(
    ...
    targets: [
        .binaryTarget(
            name: "IdensicMobileSDK",
            url: "\(release)/IdensicMobileSDK_Core-\(version).zip",
            checksum: checksums.msdk
        ),
        .binaryTarget(
            name: "IdensicMobileSDK_MRTDReader",
            url: "\(release)/IdensicMobileSDK_MRTDReader-\(version).zip",
            checksum: checksums.mrtd
        ),
        .binaryTarget(
            name: "IdensicMobileSDK_VideoIdent",
            url: "\(release)/IdensicMobileSDK_VideoIdent-\(version).zip",
        ...
        .target(
            ...
            path: "Modules/MRTDReaderWrapper"
        ),
        .target(
            ...
            path: "Modules/VideoIdentWrapper"
        )
    ]
)